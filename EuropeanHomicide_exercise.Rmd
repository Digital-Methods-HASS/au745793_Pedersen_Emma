...
Western_Europe$`Homicide rate in Europe over long-term (per 100,000) (homicides per 100,000 people)`
...
```
#Ved at bruge ovenstående funktion kan vi tjekke at navnet på fjerde kolonne er blevet ændret
...
The homicides rate fall and reign duration of the monarch rises. 

```{r eval = TRUE}
# YOUR CODE HERE:
kings <- read_csv2("data/w10_kings_clean.csv")
names(kings)

kings %>%
  mutate(duration=end_of_reign-start_of_reign,midyear=end_of_reign-duration/2) %>% 
...
Answer: As I can see we are definitely more civilized today. The tables show how the homicide rates in all the researched countries has decreased greatly during the years. 