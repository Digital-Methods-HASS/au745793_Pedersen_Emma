...
# Answer:The order is longitude first and then latitude second
...
# The layers box changes from a box you have to open into an all-time open box visualizing the layers
...
places <- read_sheet("https://docs.google.com/spreadsheets/d/1PlxsPElZML8LZKyXbqdAYeQCDIvDps2McZx1cTVWSzI/edit#gid=124710918",
                     ...
                     range = "DigitalMethods")  # select the correct worksheet name
   
glimpse(places)  
# Question 3: are the Latitude and Longitude columns present? 
# Do they contain numeric decimal degrees?
# Answer:Yes they are and they are numeric dbl
...
DanMap<-leaflet() %>% 
  ...
  addMarkers(lng = places$Longitude, 
             ...
             popup = paste(places$Description, "<br>", places$Type,places$Place_Name,places$Notes))%>%
  addLayersControl(baseGroups = names(esri),
                   options = layersControlOptions(collapsed = FALSE)) %>%
  addMiniMap(tiles = esri[[1]], toggleDisplay = TRUE,
             position = "bottomright") %>%
...
# Solution
#clusterOptions = markerClusterOptions, in the addMarkers()funktion
...
# Your brief answer: Clustering makes it easier to manage when you have a lot of points/places in the same area 
...
# Solution 
#places$Description,places$Type,places$Place_Name,places$Notes in the popup=paste()
